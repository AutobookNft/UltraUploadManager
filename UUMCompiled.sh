...
output_dir="/home/fabio/sandbox/UltraUploadSandbox/UUCompiled"

# Nomi dei file di output divisi per categoria
server_file="$output_dir/UUM_server_code.txt"
client_file="$output_dir/UUM_client_code.txt"
config_file="$output_dir/UUM_config_misc.txt"

log_file="$output_dir/UUM_compilation_log.txt"
timestamp=$(date +"%Y%m%d%H%M%S")
zip_file="$output_dir/UUM_compiled_$timestamp.zip"
hash_file="$output_dir/UUM_file_hashes.txt"
prev_hash_file="$output_dir/UUM_file_hashes_prev.txt"
shared_dir="/var/www/shared"
modified_files_file="$shared_dir/UUM_modified_files.txt"

# Directory target specifica per UUM
target_dir="/home/fabio/sandbox/UltraUploadSandbox/packages/ultra/uploadmanager"
...
generate_hashes() {
    echo "### Hash dei File Inclusi in UUM ###" > "$hash_file"
    find "$target_dir" \
...
compare_hashes() {
    if [ -f "$prev_hash_file" ]; then
        echo "### File Modificati in UUM ###" > "$modified_files_file"
        ...
        echo "Nessun file hash precedente trovato per UUM." > "$modified_files_file"
    fi
}

# Inizia il log
echo "Inizio compilazione UUM: $(date)" > "$log_file"
server_count=0
client_count=0
config_count=0
excluded_count=0
...
echo -e "######## Ultra Upload Manager (UUM) - Codice Server - Generato il $(date) ########\n" > "$server_file"
echo -e "######## Ultra Upload Manager (UUM) - Codice Client - Generato il $(date) ########\n" > "$client_file"
echo -e "######## Ultra Upload Manager (UUM) - Configurazioni e Altro - Generato il $(date) ########\n" > "$config_file"
...
echo "Fine compilazione UUM: $(date)" >> "$log_file"
...
echo "Compilazione UUM completata!"
