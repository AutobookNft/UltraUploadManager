...
output_dir="/home/fabio/libraries/USMCompiled"

# Nomi dei file di output divisi per categoria
server_file="$output_dir/USM_server_code.txt"
client_file="$output_dir/USM_client_code.txt"
config_file="$output_dir/USM_config_misc.txt"

log_file="$output_dir/USM_compilation_log.txt"
timestamp=$(date +"%Y%m%d%H%M%S")
zip_file="$output_dir/USM_compiled_$timestamp.zip"
hash_file="$output_dir/USM_file_hashes.txt"
prev_hash_file="$output_dir/USM_file_hashes_prev.txt"
shared_dir="/var/www/shared"
modified_files_file="$shared_dir/USM_modified_files.txt"

# Directory target specifica per USM - percorso corretto
target_dir="/home/fabio/sandbox/UltraUploadSandbox"
...
generate_hashes() {
    echo "### Hash dei File Inclusi in USM ###" > "$hash_file"
    find "$target_dir" \
        ...
        -type f \( -name "*.php" -o -name "*.ts" -o -name "*.js" -o -name "*.env" -o -name "*.config.js" -o -name "*.blade.php" -o -name "*.css" -o -name "*.ico" -o -name "*.txt" -o -name "*.json" -o -name "*.stub" \) -print | sort | while read -r file; do
            ...
            elif [[ ! $file == *"$target_dir/packages/ultra/"* ]]; then
                hash=$(sha256sum "$file" | awk '{print $1}')
                echo "$file: $hash" >> "$hash_file"
            fi
        done
...
compare_hashes() {
    if [ -f "$prev_hash_file" ]; then
        echo "### File Modificati in USM ###" > "$modified_files_file"
        ...
        echo "Nessun file hash precedente trovato per USM." > "$modified_files_file"
    fi
}

# Inizia il log
echo "Inizio compilazione USM: $(date)" > "$log_file"
server_count=0
client_count=0
config_count=0
excluded_count=0
...
echo -e "######## Ultra Sandbox (USM) - Codice Server - Generato il $(date) ########\n" > "$server_file"
echo -e "######## Ultra Sandbox (USM) - Codice Client - Generato il $(date) ########\n" > "$client_file"
echo -e "######## Ultra Sandbox (USM) - Configurazioni e Altro - Generato il $(date) ########\n" > "$config_file"
...
find "$target_dir" \
    ...
    -type f \( -name "*.php" -o -name "*.ts" -o -name "*.js" -o -name "*.env" -o -name "*.config.js" -o -name "*.blade.php" -o -name "*.css" -o -name "*.ico" -o -name "*.txt" -o -name "*.json" -o -name "*.stub" \) -print | sort | while read -r file; do
        ...
        # Escludi tutti i file sotto packages/ultra/ tranne authsandbox
        if [[ $file == *"$target_dir/packages/ultra/"* && ! $file == *"$target_dir/packages/ultra/authsandbox"* ]]; then
            excluded_files+=("$file")
            ((excluded_count++))
            continue
        fi
...
echo "Fine compilazione USM: $(date)" >> "$log_file"
...
echo "Compilazione USM completata!"
