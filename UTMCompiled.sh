...
output_dir="/home/fabio/sandbox/UltraUploadSandbox/UTMCompiled"

# Nomi dei file di output divisi per categoria
server_file="$output_dir/UTM_server_code.txt"
client_file="$output_dir/UTM_client_code.txt"
config_file="$output_dir/UTM_config_misc.txt"

log_file="$output_dir/UTM_compilation_log.txt"
timestamp=$(date +"%Y%m%d%H%M%S")
zip_file="$output_dir/UTM_compiled_$timestamp.zip"
hash_file="$output_dir/UTM_file_hashes.txt"
prev_hash_file="$output_dir/UTM_file_hashes_prev.txt"
shared_dir="/var/www/shared"
modified_files_file="$shared_dir/UTM_modified_files.txt"

# Directory target specifica per UTM
target_dir="/home/fabio/sandbox/UltraUploadSandbox/packages/ultra/translationmanager"
...
generate_hashes() {
    echo "### Hash dei File Inclusi in UTM ###" > "$hash_file"
    find "$target_dir" \
        -type d \( -path "*/vendor" -o -path "*/node_modules" -o -path "*/.*" -o -path "*/storage/framework" -o -path "*/public" \) -prune -o \
        -type f \( -name "*.php" -o -name "*.ts" -o -name "*.js" -o -name "*.env" -o -name "*.config.js" -o -name "*.blade.php" -o -name "*.css" -o -name "*.ico" -o -name "*.txt" \) -print | sort | while read -r file; do
...
compare_hashes() {
    if [ -f "$prev_hash_file" ]; then
        echo "### File Modificati in UTM ###" > "$modified_files_file"
        ...
        echo "Nessun file hash precedente trovato per UTM." > "$modified_files_file"
    fi
}

# Inizia il log
echo "Inizio compilazione UTM: $(date)" > "$log_file"
server_count=0
client_count=0
config_count=0
excluded_count=0
...
echo -e "######## Ultra Translation Manager (UTM) - Codice Server - Generato il $(date) ########\n" > "$server_file"
echo -e "######## Ultra Translation Manager (UTM) - Codice Client - Generato il $(date) ########\n" > "$client_file"
echo -e "######## Ultra Translation Manager (UTM) - Configurazioni e Altro - Generato il $(date) ########\n" > "$config_file"
...
find "$target_dir" \
    ...
    -type f \( -name "*.php" -o -name "*.ts" -o -name "*.js" -o -name "*.env" -o -name "*.config.js" -o -name "*.blade.php" -o -name "*.css" -o -name "*.ico" -o -name "*.txt" \) -print | sort | while read -r file; do
        ...
        if [[ $file == *".php" && $file != *".blade.php" ]]; then
            ...
            # Tutti gli altri file vanno nella configurazione/altro
            add_file_to_output "$file" "$config_file" "Config"
        fi
    done
...
echo "Fine compilazione UTM: $(date)" >> "$log_file"
...
echo "Compilazione UTM completata!"
