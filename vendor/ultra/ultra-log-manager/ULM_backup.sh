...
backup_dest_c="${backup_base_dir}/ULM_${timestamp}"  # Sottodirectory specifica per ULM
LOG_FILE="/home/fabio/libraries/ULMCompiled/ULM_backup_log.txt"

# Directory sorgente (intero progetto UltraLogManager)
source_dir="/home/fabio/libraries/UltraLogManager"
...
drive_d="/mnt/d/Il mio Drive/EGI_backup/ULM_${timestamp}"
drive_e="/mnt/e/EGI_backup/ULM_${timestamp}"
drive_h="/mnt/h/EGI_backup/ULM_${timestamp}"
...
echo "===== Inizio backup ULM $(date) =====" >> "$LOG_FILE"
...
if rsync -avz "${exclude_dirs[@]}" "$source_dir/" "$backup_dest_c"; then
    echo "✓ Backup principale ULM completato in: $backup_dest_c" >> "$LOG_FILE"
else
    echo "✗ ERRORE nel backup principale ULM!" >> "$LOG_FILE"
    exit 1
fi
...
if check_drive "$drive_d" "D:"; then
    ...
    if rsync -av "$backup_dest_c/" "$drive_d"; then
        echo "✓ Copia ULM su D: completata in $drive_d" >> "$LOG_FILE"
    else
        echo "✗ ERRORE nella copia ULM su D:" >> "$LOG_FILE"
    fi
fi
...
if check_drive "$drive_e" "E:"; then
    ...
    if rsync -av "$backup_dest_c/" "$drive_e"; then
        echo "✓ Copia ULM su E: completata in $drive_e" >> "$LOG_FILE"
    else
        echo "✗ ERRORE nella copia ULM su E:" >> "$LOG_FILE"
    fi
fi
...
if check_drive "$drive_h" "H:"; then
    ...
    if rsync -av "$backup_dest_c/" "$drive_h"; then
        echo "✓ Copia ULM su H: completata in $drive_h" >> "$LOG_FILE"
    else
        echo "✗ ERRORE nella copia ULM su H:" >> "$LOG_FILE"
    fi
fi

# Fine log operazione
echo "===== Fine backup ULM $(date) =====" >> "$LOG_FILE"