...
output_dir="/home/fabio/libraries/ULMCompiled"

# Nomi dei file di output divisi per categoria
server_file="$output_dir/ULM_server_code.txt"
client_file="$output_dir/ULM_client_code.txt"
config_file="$output_dir/ULM_config_misc.txt"

log_file="$output_dir/ULM_compilation_log.txt"
timestamp=$(date +"%Y%m%d%H%M%S")
zip_file="$output_dir/ULM_compiled_$timestamp.zip"
hash_file="$output_dir/ULM_file_hashes.txt"
prev_hash_file="$output_dir/ULM_file_hashes_prev.txt"
shared_dir="/var/www/shared"
modified_files_file="$shared_dir/ULM_modified_files.txt"

# Directory target specifica per ULM (vedo che il percorso sembra essere in WSL: Ubuntu)
target_dir="/home/fabio/libraries/UltraLogManager"
...
generate_hashes() {
    echo "### Hash dei File Inclusi in ULM ###" > "$hash_file"
    find "$target_dir" \
        -type d \( -path "*/vendor" -o -path "*/node_modules" -o -path "*/.*" -o -path "*/storage/framework" -o -path "*/public" \) -prune -o \
        -type f \( -name "*.php" -o -name "*.ts" -o -name "*.js" -o -name "*.env" -o -name "*.config.js" -o -name "*.blade.php" -o -name "*.css" -o -name "*.ico" -o -name "*.txt" -o -name "*.json" \) -print | sort | while read -r file; do
...
compare_hashes() {
    if [ -f "$prev_hash_file" ]; then
        echo "### File Modificati in ULM ###" > "$modified_files_file"
        ...
        echo "Nessun file hash precedente trovato per ULM." > "$modified_files_file"
    fi
}

# Inizia il log
echo "Inizio compilazione ULM: $(date)" > "$log_file"
server_count=0
client_count=0
config_count=0
excluded_count=0
...
echo -e "######## Ultra Log Manager (ULM) - Codice Server - Generato il $(date) ########\n" > "$server_file"
echo -e "######## Ultra Log Manager (ULM) - Codice Client - Generato il $(date) ########\n" > "$client_file"
echo -e "######## Ultra Log Manager (ULM) - Configurazioni e Altro - Generato il $(date) ########\n" > "$config_file"
...
find "$target_dir" \
    ...
    -type f \( -name "*.php" -o -name "*.ts" -o -name "*.js" -o -name "*.env" -o -name "*.config.js" -o -name "*.blade.php" -o -name "*.css" -o -name "*.ico" -o -name "*.txt" -o -name "*.json" \) -print | sort | while read -r file; do
        ...
        if [[ $file == *".php" && $file != *".blade.php" ]]; then
            ...
            # Tutti gli altri file vanno nella configurazione/altro (incluso composer.json)
            add_file_to_output "$file" "$config_file" "Config"
        fi
    done
...
echo "Fine compilazione ULM: $(date)" >> "$log_file"
...
echo "Compilazione ULM completata!"
